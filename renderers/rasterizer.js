...
import { addListeners, addInputInterval } from "./utils/listeners.js"
...
function setGLParameters(gl) {
    ...
    return {
      ...
      normalLocation,
      worldMatrixLocation,
      viewMatrixProjectionLocation,
      colorLocation,
      reverseLightDirectionLocation
    }
}

async function setTerrain(gl) {
...
async function setChunk(gl, sampler, offsetX, offsetY) {
  ...
  for (let i=0; i < 32; i++) {
    for (let j=0; j < 32; j++) {
      ...
      terrainA.push.apply(terrainA, setCube((j+offsetY)*cubeSize, pixel[0]*cubeSize, (i+offsetX)*cubeSize, terrainA.length/3));
    }
  }
  ...
  for (let i=0; i < 32; i++) {
    for (let j=0; j < 32; j++) {
      ...
      var diff = (height - Math.min(heightF, heightB, heightL, heightR)) / cubeSize;
      terrainB.push.apply(terrainB, topFace    ((i+offsetX)*cubeSize, height, (j+offsetY)*cubeSize, terrainB.length/3));
      terrainB.push.apply(terrainB, bottomFace ((i+offsetX)*cubeSize, height, (j+offsetY)*cubeSize, terrainB.length/3));
      
      if (AddrightFace == true) terrainB.push.apply(terrainB, rightFace ((i+offsetX)*cubeSize, height, (j+offsetY)*cubeSize, terrainB.length/3));
      if (AddleftFace  == true) terrainB.push.apply(terrainB, leftFace  ((i+offsetX)*cubeSize, height, (j+offsetY)*cubeSize, terrainB.length/3));
      if (AddfrontFace == true) terrainB.push.apply(terrainB, frontFace ((i+offsetX)*cubeSize, height, (j+offsetY)*cubeSize, terrainB.length/3));
      if (AddbackFace  == true) terrainB.push.apply(terrainB, backFace  ((i+offsetX)*cubeSize, height, (j+offsetY)*cubeSize, terrainB.length/3));

      for (let k=0; k < diff-1; k++) {
        terrainB.push.apply(terrainB, topFace  ((i+offsetX)*cubeSize,  (height/cubeSize-k)*cubeSize, (j+offsetY)*cubeSize, terrainB.length/3));
        terrainB.push.apply(terrainB, rightFace((i+offsetX)*cubeSize,  (height/cubeSize-k)*cubeSize, (j+offsetY)*cubeSize, terrainB.length/3));
        terrainB.push.apply(terrainB, leftFace ((i+offsetX)*cubeSize,  (height/cubeSize-k)*cubeSize, (j+offsetY)*cubeSize, terrainB.length/3));
        terrainB.push.apply(terrainB, frontFace((i+offsetX)*cubeSize,  (height/cubeSize-k)*cubeSize, (j+offsetY)*cubeSize, terrainB.length/3));
        terrainB.push.apply(terrainB, backFace ((i+offsetX)*cubeSize,  (height/cubeSize-k)*cubeSize, (j+offsetY)*cubeSize, terrainB.length/3));
      }
      if (diff > 0) { 
        terrainB.push.apply(terrainB, bottomFace((i+offsetX)*cubeSize, (height/cubeSize-diff+1)*cubeSize, (j+offsetY)*cubeSize, terrainB.length/3));
        terrainB.push.apply(terrainB, rightFace ((i+offsetX)*cubeSize, (height/cubeSize-diff+1)*cubeSize, (j+offsetY)*cubeSize, terrainB.length/3));
        terrainB.push.apply(terrainB, leftFace  ((i+offsetX)*cubeSize, (height/cubeSize-diff+1)*cubeSize, (j+offsetY)*cubeSize, terrainB.length/3));
        terrainB.push.apply(terrainB, frontFace ((i+offsetX)*cubeSize, (height/cubeSize-diff+1)*cubeSize, (j+offsetY)*cubeSize, terrainB.length/3));
        terrainB.push.apply(terrainB, backFace  ((i+offsetX)*cubeSize, (height/cubeSize-diff+1)*cubeSize, (j+offsetY)*cubeSize, terrainB.length/3));
      }
    }
  }
...
function getChunkBox(offsetX, offsetY) {
  return []
}
